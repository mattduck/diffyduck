...
func TestGenerateCompletions(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			words:       []string{""},
			wantContain: []string{"diff", "show", "log", "branch", "status", "config", "list", "add", "edit"},
		},
		{
			name:        "partial subcommand",
			words:       []string{"di"},
		...
		},
		{
			...
			words:       []string{"help", ""},
			wantContain: []string{"diff", "show", "log", "branch", "list", "add"},
		},
		{
			name:        "help partial",
			words:       []string{"help", "b"},
		...
		},
		{
			name:        "list flags",
			words:       []string{"list", "--"},
			wantContain: []string{"--store", "--kind", "--prefix", "--since", "--status", "--all-branches"},
			wantAbsent:  []string{"--cached", "--oneline"},
		},
		{
			name:        "list --status value",
			words:       []string{"list", "--status", ""},
			wantContain: []string{"unresolved", "resolved", "all"},
		},
		{
			name:        "list --store value",
			words:       []string{"list", "--store", ""},
			wantContain: []string{"all", "db", "file"},
		},
		{
			name:        "list --kind value",
			words:       []string{"list", "--kind", ""},
			wantContain: []string{"comment", "issue", "all"},
		},
		{
			name:        "list inline --status=",
			words:       []string{"list", "--status="},
		...
		},
		{
			name:        "add flags",
			words:       []string{"add", ""},
			wantContain: []string{"-m", "--commit", "--prefix", "--type", "--scope", "--ticket"},
			wantAbsent:  []string{"--cached", "--status"},
		},
		{
			name:        "edit flags",
			words:       []string{"edit", "abc", "--"},
			wantContain: []string{"--resolved"},
			wantAbsent:  []string{"--status", "--cached"},
		},
		{
			name:        "edit completes IDs",
			words:       []string{"edit", ""},
			commentIDs:  mockCommentIDs,
			wantContain: []string{"1739380234567", "1739380299000", "1739400000000"},
		},
		{
			name:        "edit ID with prefix",
			words:       []string{"edit", "17394"},
			commentIDs:  mockCommentIDs,
			wantContain: []string{"1739400000000"},
			wantAbsent:  []string{"1739380234567", "1739380299000"},
		},
		{
			name:       "edit no IDs without store",
			words:      []string{"edit", ""},
			commentIDs: nil,
			wantAbsent: []string{"1739380234567"},
		},
		{
			name:        "resolve completes IDs",
			words:       []string{"resolve", ""},
			commentIDs:  mockCommentIDs,
			wantContain: []string{"1739380234567", "1739380299000", "1739400000000"},
		},
		{
			name:        "unresolve completes IDs",
			words:       []string{"unresolve", ""},
			commentIDs:  mockCommentIDs,
			wantContain: []string{"1739380234567", "1739380299000", "1739400000000"},
		},
		{
			...
			words:       []string{"co"},
			wantContain: []string{"config", "completion"},
