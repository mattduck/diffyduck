...
func GenerateExample(defaultKeys KeysConfig) string {
	...
	if nav := defaultKeys.Navigation; nav != nil {
		...
		writeKeys("next_comment", "next unresolved comment", nav.NextComment)
		writeKeys("prev_comment", "prev unresolved comment", nav.PrevComment)
		writeKeys("next_all_comment", "next comment (any)", nav.NextAllComment)
		writeKeys("prev_all_comment", "prev comment (any)", nav.PrevAllComment)
	...
	if a := defaultKeys.Actions; a != nil {
		...
		writeKeys("yank_unresolved", "copy unresolved comments", a.YankUnresolved)
		writeKeys("yank_all_comments", "copy all comments", a.YankAllComments)
